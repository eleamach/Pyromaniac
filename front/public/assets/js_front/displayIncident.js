...
import { getBounds, getColorByLevel, getSensorCoverage } from "./sensor.js";
...
function generateUniquePairs(arr) {
    ...
    for (let i = 0; i < arr.length - 1; i++) {
        for (let j = i + 1; j < arr.length; j++) {
            ...
            const isEvenRow1 = Math.ceil((arr[i].id_sensor - 1) / nbCol);
            const isEvenRow2 = Math.ceil((arr[j].id_sensor - 1) / nbCol);
...
export function displaySensorOff(map, sensors) {
    sensors.forEach((sensor) => {
        const id = sensor.id_sensor
        const coverage = getSensorCoverage(sensor, gap)
        const boundSensor = getBounds(coverage)
        var sensorFire = map.getLayer("sensor-off-" + id)
        if (sensorFire) {
            map.removeLayer(sensorFire.id)
            map.removeSource(sensorFire.id)
        }
        var sensorGeo = {
            type: 'Feature',
            geometry: {
